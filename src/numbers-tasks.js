...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorDotProduct = x1 * x2 + y1 * y2;
  const magnitudeVector1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitudeVector2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  const cosTheta = vectorDotProduct / (magnitudeVector1 * magnitudeVector2);
...
function isPrime(n) {
  if (n <= 1) {
    return false; // 1 та менші числа не є простими
  }

  // Перевірка простоти числа від 2 до квадратного кореня з числа
  ...
  return true; // Якщо не знайдено дільників, число є простим
}
...
function getMaxNumber(firstNumber, secondNumber) {
  // використаємо вбудовану футкцію Math.max(). що б отримати максимальне число з двох
...
function getRandomInteger(min, max) {
  // Math.random - генерує випадкове число з плаваючою комою від [0 ; 1)
  // (max - min + 1) - кількість можливих значень у заданому діапазоні
  // + min  - зсуває діапазон, щоб починався з min
...
function getHypotenuse(a, b) {
  // використаємо вбудовану ф-ію для обчислення гіпотенузи прямокутного трикутника, еквівалентну до (a ** 2 + b ** 2) ** 0.5
...
function getCountOfOddNumbers(number) {
  // Math.abs - дозволяє отримати абсолютне значення числа
  // / 2 дозволяє отримати к - сть непарних чисел
  // Math.round - округлити результат до найближчого цілого числа
